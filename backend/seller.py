    ...
    def forward(self, batch):
        ...
        signals = {
            ...
            'price_coordination': self.price_coordinator(
                seller_embeddings,
                batch['seller_features']  # Contains temporal info
            ),
...
class SellerNetworkTrainer:
    ...
    def move_to_device(self, batch):
        ...
        return moved
    def save_training_artifacts(self, train_dataset, val_dataset):
        """Save all training artifacts needed for inference"""

        # 1. Save feature extractor
        ...
        for i in range(len(train_dataset)):
            ...
            all_features.append(sample['seller_features'].numpy())
        ...
        feature_extractor = {
            'scaler': scaler,
            'feature_names': train_dataset.feature_names,  # Add this attribute to dataset
            'feature_stats': {
        ...
        with torch.no_grad():
            for batch in val_loader:
                ...
                seller_embeddings = self.model.seller_embedding(batch['seller_features'])

                # Store embeddings
                for i, seller_id in enumerate(batch['seller_id']):
                    embeddings[seller_id] = seller_embeddings[i].cpu().numpy()

        graph_embeddings = {
            'embedding_dim': self.config.embedding_dim,
            'embeddings': embeddings,
            'model_version': self.best_f1
        }
...
class SellerNetworkInterface:
    ...
    def __init__(self, model_path: str, config: SellerNetworkConfig, device='cuda'):
        ...
        add_safe_globals([SellerNetworkConfig])  # No self-import
        # Load model
        ...
        self.feature_extractor = joblib.load("models/seller/feature_extractor.pkl")
        self.normalization_factors = self.feature_extractor.get("normalization_factors", {})
    ...
    def analyze_seller(self, seller_data: Dict) -> Dict:
        ...
        with torch.no_grad():
            outputs = self.model({'seller_features': features_tensor})
    
        result = {
            "seller_id": seller_data.get("seller_id", "unknown"),
            "fraud_probability": outputs["fraud_probability"].squeeze().item(),
            "network_member": outputs["is_network_member_pred"].squeeze().item(),
            "price_coordination": outputs["has_price_coordination_pred"].squeeze().item(),
            "inventory_sharing": outputs["has_inventory_sharing_pred"].squeeze().item(),
            "exit_scam_risk": outputs["exit_scam_risk_pred"].squeeze().item(),
            "network_signals": {
                "connected_sellers": [],
    ...
    def prepare_seller_features(self, seller_data: dict) -> np.ndarray:
        """Extracts and normalizes seller features from input data"""
        # Same normalization factors used during training
        normalization = self.normalization_factors

        metrics = seller_data.get('seller_metrics', {})
        pricing = seller_data.get('pricing_behavior', {})
        inventory = seller_data.get('inventory_patterns', {})
        network = seller_data.get('network_features', {})
        temporal = seller_data.get('temporal_features', {})

        features = [
            metrics.get('total_products', 0) / normalization.get('total_products', 1000),
            metrics.get('avg_product_price', 0) / normalization.get('avg_product_price', 100),
            metrics.get('avg_rating', 0) / normalization.get('avg_rating', 5),
            metrics.get('response_time_hours', 0) / normalization.get('response_time_hours', 24),
            metrics.get('fulfillment_rate', 0),
            metrics.get('return_rate', 0),
            metrics.get('customer_complaints', 0) / normalization.get('customer_complaints', 100),

            pricing.get('avg_price_change_frequency_days', 0) / normalization.get('avg_price_change_frequency_days', 30),
            pricing.get('max_price_drop_percent', 0) / normalization.get('max_price_drop_percent', 100),
            pricing.get('synchronized_changes_count', 0) / normalization.get('synchronized_changes_count', 50),
            float(pricing.get('competitor_price_matching', False)),
            float(pricing.get('dynamic_pricing_detected', False)),

            inventory.get('avg_stock_level', 0) / normalization.get('avg_stock_level', 1000),
            inventory.get('max_stock_spike', 0) / normalization.get('max_stock_spike', 10000),
            inventory.get('inventory_turnover_days', 0) / normalization.get('inventory_turnover_days', 30),

            network.get('shared_product_count', 0) / normalization.get('shared_product_count', 100),
            network.get('address_similarity_score', 0),
            network.get('customer_overlap_score', 0),
            network.get('reviewer_overlap_score', 0),

            seller_data.get('account_age_days', 0) / normalization.get('account_age_days', 365),
            temporal.get('registration_hour', 0) / normalization.get('registration_hour', 24)
        ]
...
def main():
    ...
    trainer.save_training_artifacts(train_dataset, val_dataset)
